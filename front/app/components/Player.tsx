...
export default function Player() {
  ...
  useEffect(() => {
    ...
    if (currentTrack) {
      if (audioRef.current) {
        const audioUrl = `http://localhost:3001${currentTrack.audioUrl}`;
  ...
  const handleLike = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3001/api/tracks/${currentTrack.id}/like`,
        {},
        {
          headers: {
            ...
            <div className="flex items-center space-x-4">
              <div className="w-16 h-16 bg-gray-700 rounded overflow-hidden">
                {currentTrack?.coverUrl && (
                  <img 
                    src={`http://localhost:3001${currentTrack.coverUrl}`} 
