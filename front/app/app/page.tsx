...
export default function Home() {
  ...
  const fetchLatestTracks = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3001/api/tracks/latest', {
        headers: token ? {
  ...
  const handleLike = async (trackId: string) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://localhost:3001/api/tracks/${trackId}/like`,
        {},
        {
          headers: {
  ...
  const handleRepost = async (trackId: string) => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:3001/api/tracks/${trackId}/repost`,
        {},
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`
            ...
            {tracks.map((track) => (
              ...
              >
                <div className="aspect-square relative">
                  {track.coverUrl ? (
                    <img
                      src={`http://localhost:3001${track.coverUrl}`}
                      alt={track.title}
